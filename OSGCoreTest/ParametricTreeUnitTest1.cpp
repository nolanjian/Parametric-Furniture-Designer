...
using namespace PFDComponent;

TEST(ParametricTree, BuildTree) {
	// Top Level
	osg::ref_ptr<Parametric> pTop = new Parametric();
	...
	// Second Level
	osg::ref_ptr<Parametric> pSecond1 = new Parametric();
	...
	// Second Level
	osg::ref_ptr<Parametric> pSecond2 = new Parametric();
...
TEST(ParametricTree, CalTest) {
	osg::ref_ptr<Parametric> pc = new Parametric();
...
TEST(ParametricTree, RegexTest1)
{
	...
	Parametric::regexParseFormular(L" A = 800 ", strKey, strVal);
...
{
	std::wstring strKey, strVal;
	Parametric::regexParseFormular(L"     \r\n       B       \r\n      =               \r\n           800        \r\n         ",
		strKey, strVal);
...
{
	std::wstring strKey, strVal;
	Parametric::regexParseFormular(L" W=800 ", strKey, strVal);
...
{
	std::wstring strKey, strVal;
	Parametric::regexParseFormular(L"Q=800", strKey, strVal);
...
{
	std::wstring str(L"   \r\n   \r\n     \r\n");
	bool bRet = Parametric::regexParseKV(str);
	EXPECT_EQ(bRet, false);

	std::wstring str1(L"");
	bool bRet1 = Parametric::regexParseKV(str1);
...
{
	std::wstring strKey, strVal;
	bool bRet = Parametric::regexParseFormular(L"Q", strKey, strVal);
	EXPECT_EQ(bRet, false);

	bool bRet1 = Parametric::regexParseFormular(L"Q=", strKey, strVal);
	EXPECT_EQ(bRet1, false);
}
