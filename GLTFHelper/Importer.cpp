...
namespace PFD_GLTF_Helper
{
	...
	osg::ref_ptr<osg::Group> Importer::Load(std::shared_ptr<fx::gltf::Document> gltf)
	{
		...
		osg::ref_ptr<PFDComponent::BaseObject> pScene = new PFDComponent::BaseObject();
	...
	void Importer::AddNodes(osg::ref_ptr<PFDComponent::BaseObject> pNode, std::shared_ptr<fx::gltf::Document> gltfObject, const std::vector<int32_t>& nodes)
	{
		for (int32_t childID : nodes)
		{
			const fx::gltf::Node& childNode = gltfObject->nodes[childID];

			osg::ref_ptr<PFDComponent::BaseObject> pChild = PFDComponent::IComponentFactory::Get()->CreateObject(GetClassType(childNode));
			if (pChild == nullptr)
			{
				continue;
			}
	...
	PFDComponent::IComponentFactory::Type Importer::GetClassType(const fx::gltf::Node& node)
	{
		if (node.extensionsAndExtras.contains("class"))
		{
			return (PFDComponent::IComponentFactory::Type)node.extensionsAndExtras["class"].get<int>();
		}

		return PFDComponent::IComponentFactory::Type::BASE_OBJECT;
	}
	...
	void Importer::InitFromGLTFNode(osg::ref_ptr<PFDComponent::BaseObject> pNode, std::shared_ptr<fx::gltf::Document> gltfObject, const fx::gltf::Node& curNode)
	{
...
#pragma endregion LOAD_MATRIX
	...
	bool Importer::ImportMesh(osg::ref_ptr<PFDComponent::BaseObject> pNode, std::shared_ptr<fx::gltf::Document> gltfObject, const fx::gltf::Node& curNode)
	{
		if (curNode.mesh == -1)
		{
			return true;
