...
namespace PFD
{
	namespace GLTF
	{
		...
		osg::ref_ptr<osg::Group> ImporterImp::Load(const std::wstring& path)
		{
		...
		osg::ref_ptr<osg::Group> ImporterImp::Load(const std::string& utf8Json)
		{
			std::stringstream	ss;
			ss << utf8Json;
		...
		osg::ref_ptr<osg::Group> ImporterImp::Load(std::shared_ptr<fx::gltf::Document> gltf)
		{
			if (!m_pDoc)
			{
				return nullptr;
		...
		void ImporterImp::AddNodes(osg::ref_ptr<PFD::Component::BaseObject> pNode, std::shared_ptr<fx::gltf::Document> gltfObject, const std::vector<int32_t>& nodes)
		{
		...
		PFD::Component::IComponentFactory::Type ImporterImp::GetClassType(const fx::gltf::Node& node)
		{
		...
		void ImporterImp::InitFromGLTFNode(osg::ref_ptr<PFD::Component::BaseObject> pNode, std::shared_ptr<fx::gltf::Document> gltfObject, const fx::gltf::Node& curNode)
		{
...
#pragma endregion LOAD_MATRIX
		...
		bool ImporterImp::ImportMesh(osg::ref_ptr<PFD::Component::BaseObject> pNode, std::shared_ptr<fx::gltf::Document> gltfObject, const fx::gltf::Node& curNode)
		{
			if (curNode.mesh == -1)
			{
				return true;
